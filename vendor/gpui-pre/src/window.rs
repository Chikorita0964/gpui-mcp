...
pub struct Window {
    ...
    // gpui-mcp patch (C05): the last platform-reported position this window
    // adopted, so a stale platform position cannot cancel a synthetic hover.
...
impl Window {
    pub(crate) fn new(
        ...
        // gpui-mcp patch (C05): remember the platform position we start from.
...
impl Window {
    ...
    pub fn bounds_changed(&mut self, cx: &mut App) {
        ...
        // gpui-mcp patch (C05): adopt the platform's position only when the
        // physical mouse actually moved. A stale platform position must not
        // cancel a synthetic hover before the physical mouse moves.
