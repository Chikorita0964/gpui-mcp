...
| `cargo check -p gpui-mcp-html --tests` | fails: `tests/runtime.rs:247` (`focus_observed_element`) and `:662` (`replace_input_text`) still call fork APIs |
...
| **P-C** Pointer ownership | Synthetic pointer events run GPUI's native pipeline (`Window::dispatch_event`), and `DispatchEventResult` is public | `bounds_changed` still overwrites the synthetic pointer position with the platform's, so a resize or DPI change mid-hover reverts to the physical pointer | `window.rs:2691` |
...
| HTML runtime test target | `crates/gpui-mcp-html/tests/runtime.rs:247`, `:662` | Still calls `focus_observed_element` and `replace_input_text`; rewiring needs the P-B focus mapping and input-handler exposure |
